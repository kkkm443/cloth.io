...
import { nearbyBins as findNearby, hasCoordinates, originLabel } from './bin-model.js';
"use client";
...
import { issueLabels, issueWeights, riskScore, riskLabel, distanceMeters } from './keeper-features.js';
...
export default function ReportForm({ open, onOpenChange, onSaved, initialBin = null, maintenance = false }) {
    ...
    const [issues, setIssues] = useState({}), [mask, setMask] = useState(false), [candidates, setCandidates] = useState([]), [binId, setBinId] = useState('');
    const [step, setStep] = useState(0), [photo, setPhoto] = useState(null), [url, setUrl] = useState(''), [processing, setProcessing] = useState(false);
    const [category, setCategory] = useState('uncertain'), [address, setAddress] = useState(''), [description, setDescription] = useState(''), [point, setPoint] = useState(null), [consent, setConsent] = useState(false);
    ...
    const nearbyBins = useMemo(() => findNearby(catalog.bins, point), [catalog.bins, point]);
    const linkedBin = catalog.byId.get(binId) || null;
    function changePoint(p) {
        setPoint(p);
        if (linkedBin && hasCoordinates(linkedBin) && (!p || distanceMeters(p, [linkedBin.latitude, linkedBin.longitude]) > 50)) {
            setBinId('');
            toast.error('기존 수거함에서 위치가 달라져 연결을 해제했어요. 같은 수거함을 다시 선택해 주세요.');
        }
    }
    ...
    useEffect(() => {
        if (open) {
            ...
            setIssues({});
            setMask(false);
            setBinId(initialBin?.id || '');
            setCandidates([]);
            setAddress(initialBin?.address || '');
            setDescription(maintenance ? '수거함과 주변이 정리된 것을 현장에서 확인했어요.' : '');
            setPoint(initialBin && hasCoordinates(initialBin) ? [initialBin.latitude, initialBin.longitude] : null);
            setConsent(false);
            setAi('idle');
            setSuggested(null);
            setError('');
    ...
    async function select(file) {
        ...
        try {
            ...
            if (gps && !binId) {
                setPoint(gps);
                toast.success('사진의 GPS 위치를 불러왔어요. 실제 수거함 위치인지 확인해 주세요.');
            }
            else if (gps && linkedBin && hasCoordinates(linkedBin) && distanceMeters(gps, [linkedBin.latitude, linkedBin.longitude]) > 50) {
                toast.error('사진 GPS와 선택한 수거함 위치가 달라요. 선택한 수거함은 유지했으니 사진과 위치를 확인해 주세요.');
            }
            await maskPhoto(p, g);
        }
        catch (e) {
    ...
    function locate() {
        if (!navigator.geolocation) {
            toast.error('위치 기능을 지원하지 않는 브라우저예요.');
            return;
        }
        setLocating(true);
        navigator.geolocation.getCurrentPosition(p => { changePoint([p.coords.latitude, p.coords.longitude]); setLocating(false); toast.success('현재 위치를 표시했어요. 수거함 위치와 주소를 확인해 주세요.'); }, () => { setLocating(false); toast.error('위치를 가져오지 못했어요. 지도에서 선택하거나 주소를 입력해 주세요.'); }, { enableHighAccuracy: true, timeout: 12000 });
    }
    async function save() {
        if (!photo || busy || !canSavePhoto(privacy, reviewed, processing, mask))
            return;
    ...
    return h(Dialog, { open: open, onOpenChange: v => {
        ...
        h(DialogContent, { className: "report-dialog", onInteractOutside: e => e.preventDefault() },
                ...
                h("fieldset", { disabled: busy || !!savedReport, style: { border: 0, padding: 0, margin: 0, minWidth: 0 } },
                    ...
                    step === 1 && h(Fragment, null,
                        ...
                        h("div", { className: "field-label-row" },
                            h("label", { className: "field-label", htmlFor: "address" },
                                "\uBC1C\uACAC\uD55C \uC704\uCE58 ",
                                h("span", null, "\uD544\uC218")),
                            h("button", { className: "text-button", type: "button", disabled: locating, onClick: locate },
                                locating ? h(LoaderCircle, { size: 16, className: "spin" }) : h(LocateFixed, { size: 16 }),
                                "\uD604\uC7AC \uC704\uCE58")),
                        h("input", { id: "address", className: "text-input", maxLength: 200, value: address, onChange: e => setAddress(e.target.value), placeholder: "\uC608: \uC11C\uC6B8 \uB9C8\uD3EC\uAD6C \uB3D9\uAD50\uB85C 242, \uACE8\uBAA9 \uC785\uAD6C" }),
                        h("div", { className: "pick-map" },
                            h(MapCanvas, { compact: true, point: point, onPick: changePoint })),
                        h("div", { className: "field-hint between" },
                            h("span", null, point ? `선택한 좌표 ${point[0].toFixed(5)}, ${point[1].toFixed(5)}` : '지도를 눌러 위치를 표시할 수 있어요. 주소만 입력해도 돼요.'),
                            point && h("button", { type: "button", className: "text-button", onClick: () => changePoint(null) }, "\uC704\uCE58 \uC9C0\uC6B0\uAE30")),
                        (nearbyBins.length > 0 || linkedBin) && h(Fragment, null,
                            h("label", { className: "field-label", htmlFor: "bin-link" }, "\uAC19\uC740 \uC218\uAC70\uD568 \uC774\uB825 \uC5F0\uACB0"),
                            h("select", { id: "bin-link", className: "text-input", value: binId, onChange: e => {
                                    setBinId(e.target.value);
                                    const b = catalog.byId.get(e.target.value);
                                    if (b)
                                        setAddress(b.address);
                                } },
                                h("option", { value: "" }, "\uBCC4\uAC1C\uC758 \uC0C8 \uC218\uAC70\uD568 \u00B7 \uACF5\uACF5\uC790\uB8CC \uBBF8\uB9E4\uCE6D"),
                                linkedBin && !nearbyBins.some(b => b.id === linkedBin.id) && h("option", { value: linkedBin.id },
                                    linkedBin.address,
                                    " \u00B7 \uC120\uD0DD\uD55C \uC218\uAC70\uD568"),
                                nearbyBins.slice(0, 30).map(b => h("option", { key: b.id, value: b.id },
                                    b.address,
                                    " \u00B7 ",
                                    ...
                                    "m"))),
                            h("p", { className: "field-hint" },
                                "50m \uC774\uB0B4 \uD6C4\uBCF4\uB294 \uC790\uB3D9 \uBCD1\uD569\uD558\uC9C0 \uC54A\uC544\uC694. \uAC19\uC740 \uC218\uAC70\uD568\uC778\uC9C0 \uC9C1\uC811 \uD655\uC778\uD558\uACE0 \uC120\uD0DD\uD558\uC138\uC694.",
                                nearbyBins.length > 30 ? ' 가까운 30곳만 표시합니다. 수거함 기록에서 주소로 검색해 선택할 수도 있어요.' : '')),
            ...
            h("div", { className: "form-footer" }, step === 3 ? h(Fragment, null,
                ...
                step < 2 ? h("button", { type: "button", className: "btn primary", disabled: step === 0 ? (!photo || !canSavePhoto(privacy, reviewed, processing, mask)) : address.trim().length < 3 || ai === 'loading' || ai === 'analyzing', onClick: next },
                    step === 0 ? '상태와 위치 확인' : '제보·민원 내용 확인',
                    h(ArrowRight, { size: 17 })) : h("button", { type: "button", className: "btn primary", disabled: !consent || busy || (maintenance && (!maintenanceConfirmed || !binId)) || !canSavePhoto(privacy, reviewed, processing, mask) || (automatic && (intakeTitle.trim().length < 3 || intakeBody.trim().length < 20)), onClick: save },
