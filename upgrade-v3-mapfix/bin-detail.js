...
export default function BinDetail({ id, onClose, onReport, onMaintenance, onReportSelect }) {
                ...
                h("div", { className: "bin-detail-scroll" },
                    ...
                    tab === 'overview' && h("section", { className: "bin-info" },
                        ...
                        hasCoordinates(bin) && h("a", { className: "btn", href: `https://www.openstreetmap.org/?mlat=${bin.latitude}&mlon=${bin.longitude}#map=18/${bin.latitude}/${bin.longitude}`, target: "_blank", rel: "noreferrer" },
                            h(MapPin, { size: 16 }),
                            "\uC704\uCE58 \uD06C\uAC8C \uBCF4\uAE30")),
                    tab === 'photos' && h(ComparePhotos, { bin: bin }),
                    tab === 'history' && h("section", { className: "bin-history" },
                        ...
                        !bin.history.length ? h("div", { className: "community-empty" }, "\uC544\uC9C1 \uAD00\uCC30\u00B7\uC815\uBE44 \uAE30\uB85D\uC774 \uC5C6\uC5B4\uC694.") : bin.history.map(r => h("article", { className: `bin-history-entry ${r.kind === 'maintenance' ? 'maintenance' : ''}`, key: r.id },
                    ...
                    tab === 'community' && h(Community, { key: bin.id, bin: bin, onMaintenance: () => leave(() => onMaintenance(bin)) })),
...
function ComparePhotos({ bin }) {
    const initial = defaultComparison(bin.history), [before, setBefore] = useState(initial.before), [after, setAfter] = useState(initial.after);
