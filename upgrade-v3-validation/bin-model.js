...
export function buildCatalog(publicBins = [], reports = [], registry = {}, citizenBins = {}) {
    const map = new Map();
    for (const b of publicBins)
        map.set(b.id, { ...b, title: binTitle(b), origin: 'public', history: [] });
    for (const [id, b] of Object.entries(registry || {}))
        if (!map.has(id))
            map.set(id, { ...b, id, title: binTitle(b), name: '', province: '', district: '', origin: 'registry', history: [] });
    for (const [id, b] of Object.entries(citizenBins || {}))
        if (!map.has(id))
            map.set(id, { ...b, id, title: binTitle(b), name: '', province: '', district: '', origin: 'citizen', history: [] });
    for (const r of reports) {
        if (r.sample)
            continue;
        const id = r.bin_id || r.id;
        let b = map.get(id);
        if (!b) {
            b = { id, address: r.address, title: binTitle(r), name: '', province: '', district: '', latitude: r.latitude, longitude: r.longitude, origin: 'citizen', history: [] };
            map.set(id, b);
        }
        b.history.push(r);
    }
    for (const b of map.values()) {
        b.history.sort(compareObservations);
        b.latest = b.history[0] || null;
        b.score = observationScore(b.latest);
        b.level = levelOf(b.score);
        ...
        if (!hasCoordinates(b)) {
            const known = b.history.find(hasCoordinates);
            if (known) {
                b.latitude = known.latitude;
                b.longitude = known.longitude;
                b.coordinateStatus = 'citizen';
            }
        }
    }
    return [...map.values()];
...
export function relativeTime(t, now = Date.now()) { if (!t)
    return '기록 없음'; const delta = Math.max(0, now - t); return delta < 60000 ? '방금 전' : delta < 3600000 ? `${Math.floor(delta / 60000)}분 전` : delta < 86400000 ? `${Math.floor(delta / 3600000)}시간 전` : `${Math.floor(delta / 86400000)}일 전`; }
...
export function nearbyBins(bins, point, maxDistance = 50) { if (!point)
