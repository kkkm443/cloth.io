...
export default function Community({ bin, onMaintenance }) {
    ...
    return h("section", { className: "bin-community" },
        ...
        h("div", { className: "community-posts" }, posts.slice(0, limit).map(post => h(CommunityPost, { key: post.id, post: post, replies: items.filter(x => x.parent_id === post.id).reverse(), binId: bin.id, operator: operator }))),
        posts.length > limit && h("button", { className: "btn", onClick: () => setLimit(n => n + 20) }, "\uC774\uC804 \uAC8C\uC2DC\uAE00 \uB354 \uBCF4\uAE30"));
}
function CommunityPost({ post, replies, binId, operator }) {
    ...
    return h("article", { className: "community-post" },
